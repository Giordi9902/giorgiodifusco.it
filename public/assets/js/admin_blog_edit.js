// Blog edit page logic extracted from app/Views/admin/blog_edit.php
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const initialCourseId = courseSelect
    ? courseSelect.getAttribute("data-initial-course-id") || null
    : null;
  const initialSubjectId = subjectSelect
    ? subjectSelect.getAttribute("data-initial-subject-id") || null
    : null;
  ...
  (async () => {
    ...
    if (initialCourseId) {
      await loadSubjects(initialCourseId, initialSubjectId);
    }
  })();
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      e.preventDefault();
      if (!postId || typeof BASE_URL === "undefined") return;
      ...
      try {
        ...
        if (result.success) {
          await (window.showAppAlert
            ? window.showAppAlert(
                result.message || "Articolo aggiornato con successo",
                {
            ...
            : Promise.resolve(
                alert(result.message || "Articolo aggiornato con successo"),
              ));
          window.location.href = `${BASE_URL}/admin/blog`;
        } else {
          await (window.showAppAlert
            ? window.showAppAlert(
                result.message || "Errore durante l'aggiornamento",
                {
                  title: "Errore",
                  variant: "error",
                },
        ...
        await (window.showAppAlert
          ? window.showAppAlert(
              "Errore di rete durante l'aggiornamento dell'articolo",
              {
                title: "Errore di rete",
                variant: "error",
              },
          ...
          : Promise.resolve(
              alert("Errore di rete durante l'aggiornamento dell'articolo"),
            ));
      }
    });
  }
