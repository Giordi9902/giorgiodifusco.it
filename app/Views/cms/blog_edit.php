    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        <form id="blogPostForm"
            ...
            <div>
                ...
                <input type="text" name="title" required
                    ...
                    class="block w-full border border-slate-700 bg-slate-950/50 rounded-lg shadow-sm p-2.5 text-sm text-slate-200 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none transition">
            </div>
            ...
            <div>
                ...
                <textarea name="excerpt" rows="2"
                    class="block w-full border border-slate-700 bg-slate-950/50 rounded-lg shadow-sm p-2.5 text-sm text-slate-200 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none transition"><?= htmlspecialchars($post['excerpt'] ?? '') ?></textarea>
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-300 mb-1">Copertina articolo</label>
                <p class="text-[11px] text-slate-500 mb-2">Immagine mostrata nelle card del blog e come anteprima social. Non finisce nel testo dell'articolo.</p>
                <div class="flex items-center gap-3">
                    <div id="coverPreviewWrap" class="shrink-0 w-32 h-[4.5rem] rounded-lg overflow-hidden border border-slate-700 bg-slate-950/50 flex items-center justify-center">
                        <img id="coverPreviewImg"
                             <?= !empty($featuredImage['path']) ? 'src="' . htmlspecialchars(BASE_URL . '/' . $featuredImage['path']) . '"' : '' ?>
                             alt="" class="w-full h-full object-cover <?= empty($featuredImage['path']) ? 'hidden' : '' ?>">
                        <span id="coverPreviewPlaceholder" class="text-[10px] text-slate-500 px-2 text-center <?= empty($featuredImage['path']) ? '' : 'hidden' ?>">Nessuna copertina</span>
                    </div>
                    <div class="flex flex-col gap-1.5">
                        ...
                        <button type="button" id="removeCoverButton"
                            class="px-3 py-1.5 rounded-lg border border-slate-700 text-xs font-medium text-red-300 hover:bg-red-950/40 hover:border-red-800 transition <?= empty($featuredImage['path']) ? 'hidden' : '' ?>">
                            Rimuovi copertina
                        </button>
                    </div>
                </div>
            ...
            <div>
                ...
                <div class="flex flex-wrap items-center gap-1 mb-1.5 p-1.5 rounded-lg border border-slate-700 bg-slate-900">
                    ...
                    <button type="button" id="insertImageButton"          title="Inserisci immagine nel contenuto" class="editor-toolbar-btn">
                ...
                <textarea name="content" id="blogContent" rows="16" required
                    class="block w-full border border-slate-700 bg-slate-950/60 rounded-lg shadow-sm p-3 text-sm text-slate-200 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none transition font-mono leading-relaxed resize-y"><?= htmlspecialchars($post['content'] ?? '') ?></textarea>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="course_id" id="blogCourseSelect"
                        ...
                        class="block w-full border border-slate-700 bg-slate-950/50 rounded-lg shadow-sm p-2 text-sm text-slate-200 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none transition">
                ...
                <div class="mt-3 space-y-3">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" name="seo_title"
                                ...
                                class="block w-full border border-slate-700 bg-slate-950/50 rounded-lg shadow-sm px-2.5 py-1.5 text-[13px] text-slate-200 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none transition"
                    ...
                    <div>
                        ...
                        <textarea name="seo_description" rows="2"
                            class="block w-full border border-slate-700 bg-slate-950/50 rounded-lg shadow-sm px-2.5 py-1.5 text-[13px] text-slate-200 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none transition"><?= htmlspecialchars($post['seo_description'] ?? '') ?></textarea>
                    </div>
                </div>
            </details>
        ...
        <div class="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-xl shadow-slate-950/40 lg:sticky lg:top-24">
            <h2 class="text-sm font-semibold text-slate-100 mb-3 flex items-center gap-2">
                <span class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-purple-500/10 text-purple-300 border border-purple-500/30 text-[11px] font-mono">fx</span>
                Anteprima live
            </h2>
